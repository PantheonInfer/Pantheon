...
def build_module_graph_with_unpack_manually(model, dummy_input):
    torch_module_graph = TorchModuleGraph(model, dummy_input)
    torch_module_graph.unpack_manually()
    return torch_module_graph
...
class TorchModuleGraph(TorchGraph):
    ...
    def _extract_auxiliary_info(self):
        ...
        for node_group in self.nodes_py.nodes_op:
            ...
            if node_group.op_type in ['aten::view', 'aten::flatten', 'aten::mean', 'aten::reshape', 
                                    'aten::unsqueeze', 'aten::squeeze', 'aten::expand_as', 'aten::upsample_nearest2d', 
                                    'aten::max_pool2d', 'aten::upsample_bilinear2d', 'aten::adaptive_avg_pool2d', 'aten::reshape', 'aten::permute']:
                # get shape infor for view (aten::view) func
