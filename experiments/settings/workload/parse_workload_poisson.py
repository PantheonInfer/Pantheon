...
def parse_one_periodic(workload: dict, id):
...
def parse_one_possion(workload: dict, id):

    assert workload['deadline'] <= workload['period']
    poisson_lambda = workload['lambda']
    current_time = (workload['start'] // 33333) * 33333
    # current_time = workload['start']
    job_sequence = []
    ...
    i = 0
    while current_time < workload['end']:
        current_time += int(workload['period'] * np.random.exponential(scale=1.0 / poisson_lambda))
        # random_start_delay = random.randint(-1000, 1000)
        # if current_time + random_start_delay < 0:
        #     random_start_delay = 0
        job_sequence.append({
            ...
            'deadline': current_time + workload['deadline'],
            'shape': workload['shape'],
            'id': id
        })
        # Simulate Poisson: https://www.math.pku.edu.cn/teachers/lidf/course/stochproc/stochprocnotes/html/_book/poisson.html#def:pois-ad-xntn-expdef
...
def parse_one(workload: dict, id):
    if 'lambda' in workload:
        return parse_one_possion(workload, id)
    else:
        return parse_one_periodic(workload, id)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='indoor_smart_traffic_poisson_10.json')
